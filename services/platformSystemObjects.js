...
export function safeSystemFields(object, fields) {
  ...
  const profile = new Set(object.source_table === "users" ? ["full_name", "username", "email", "active", "created_at", "updated_at"] : ["name", "code", "address_line1", "city", "postcode", "phone", "active", "created_at", "updated_at"]);
...
export function systemWriteError(object) {
  const system = systemObject(object);
  return system ? { success: false, code: "SYSTEM_OBJECT_OPERATION_REQUIRED",
    message: "Use the existing business module to change this record.", route: system.route } : null;
}
...
export function appendSystemReadScope(object, req, clauses, params) {
  if (["product_store_stock", "inventory_movements", "purchases", "purchase_receipts", "online_orders", "sales"].includes(object?.source_table) && !object.store_scoped) {
