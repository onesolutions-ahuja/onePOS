...
export function validateRecords(records, expectedCount = 332) {
  if (records.length !== expectedCount) throw new Error(`Expected ${expectedCount} records; found ${records.length}`);
  const seen = new Set();
...
export function parseMasterCsv(text, expectedCount = 332) {
...
export async function importGlobalMaster(client, records, { expectedCount = 332, migration = null } = {}) {
  validateRecords(records, expectedCount);
