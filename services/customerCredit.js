...
export function checkPayment(currentBalanceCents, paymentAmountCents) {
  const paymentCents = Math.round(Number(paymentAmountCents) || 0);
  const balanceCents = Math.round(Number(currentBalanceCents) || 0);
  const overpayment = paymentCents > balanceCents ? paymentCents - balanceCents : 0;
    return { allowed: paymentCents <= balanceCents, overpayment };
}
...
export function generateStatement(params) {
  ...
  if (fromDate) {
    filtered = filtered.filter((tx) => new Date(tx.created_at || tx.transaction_date) >= new Date(fromDate));
  }
  if (toDate) {
    filtered = filtered.filter((tx) => new Date(tx.created_at || tx.transaction_date) <= new Date(toDate));
  }
  filtered = [...filtered].sort((a, b) => new Date(a.created_at || a.transaction_date) - new Date(b.created_at || b.transaction_date));
  let openingBalance = 0;
  if (fromDate && transactions.length) {
    const prior = transactions.filter((tx) => new Date(tx.created_at || tx.transaction_date) < new Date(fromDate));
