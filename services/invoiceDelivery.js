...
async function sendSmsViaProvider({ endpoint, apiKey, authScheme, senderId, to, body }) {
...
async function sendEmailViaProvider({ endpoint, apiKey, authScheme, from, to, subject, body }) {
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), PROVIDER_TIMEOUT_MS);
  try {
    const headers = { "Content-Type": "application/json" };
