...
/* Rebuildable movement-ledger rollup, maintained in the same transaction as
 * every store-scoped movement. */
...
ALTER TABLE stock_returns ADD COLUMN IF NOT EXISTS canonical_transaction_id UUID;
...
CREATE TABLE IF NOT EXISTS hospitality_qr_sessions (
    ...
    company_id UUID NOT NULL REFERENCES companies(id) ON DELETE CASCADE,
    store_id UUID NOT NULL REFERENCES stores(id) ON DELETE CASCADE,
    table_id UUID NOT NULL REFERENCES hospitality_tables(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS hospitality_qr_orders (
    ...
    total NUMERIC(12,2) NOT NULL DEFAULT 0,
    payment_mode VARCHAR(20) NOT NULL DEFAULT 'PAY_AT_TILL',
...
-- ============================================================
-- PAYMENT METHODS (configurable metadata-backed tender definitions)
-- ============================================================
...
('customer.credit.view', 'View Customer Credit', 'View customer credit accounts and ledger'),
('customer.credit.manage', 'Manage Customer Credit', 'Manage customer credit configuration'),
('customer.credit.payment', 'Post Customer Credit Payments', 'Post protected customer credit payments'),
('customer.credit.freeze', 'Freeze or Unfreeze Customer Credit', 'Freeze or unfreeze customer credit accounts'),
('customer.credit.statement', 'Send Customer Credit Statements', 'Send customer credit statements'),
...
/* Self-referencing relationships (for example sale -> original_transactions)
   ...
   constraint forbidding a relationship whose parent and child are the same
   object; drop it idempotently so the canonical schema and the bootstrap
   migration path agree. */
DO $$ BEGIN
...
/* platform_record_types.company_id is nullable and PostgreSQL treats NULLs as
   ...
   types can duplicate. Collapse duplicates onto one canonical row, repoint the
   references, then enforce global uniqueness explicitly. Tenant rows stay
   covered by the existing UNIQUE(object_id, company_id, record_type_key)
   constraint and the tenant partial index. */
...
CREATE TABLE IF NOT EXISTS platform_policies (id UUID PRIMARY KEY DEFAULT gen_random_uuid(),company_id UUID NOT NULL REFERENCES companies(id) ON DELETE CASCADE,api_key VARCHAR(120) NOT NULL,name VARCHAR(200) NOT NULL,policy_type VARCHAR(40) NOT NULL DEFAULT 'COMPANY_POLICY',version INTEGER NOT NULL DEFAULT 1,title VARCHAR(255) NOT NULL,body TEXT NOT NULL,status VARCHAR(20) NOT NULL DEFAULT 'DRAFT' CHECK(status IN ('DRAFT','ACTIVE','INACTIVE')),require_acceptance BOOLEAN NOT NULL DEFAULT TRUE,applicability JSONB NOT NULL DEFAULT '{}'::jsonb,display_order INTEGER NOT NULL DEFAULT 0,published_at TIMESTAMPTZ,created_by UUID REFERENCES users(id) ON DELETE SET NULL,created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),UNIQUE(company_id,api_key,version));
CREATE TABLE IF NOT EXISTS platform_policy_acceptances (id UUID PRIMARY KEY DEFAULT gen_random_uuid(),company_id UUID NOT NULL REFERENCES companies(id) ON DELETE CASCADE,policy_id UUID NOT NULL REFERENCES platform_policies(id) ON DELETE RESTRICT,policy_version INTEGER NOT NULL,user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,accepted_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),evidence JSONB NOT NULL DEFAULT '{}'::jsonb,UNIQUE(policy_id,policy_version,user_id));
CREATE TABLE IF NOT EXISTS account_action_tokens (id UUID PRIMARY KEY DEFAULT gen_random_uuid(),company_id UUID NOT NULL REFERENCES companies(id) ON DELETE CASCADE,user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,purpose VARCHAR(30) NOT NULL CHECK(purpose IN ('REGISTRATION','PASSWORD_RESET')),token_hash VARCHAR(64) NOT NULL UNIQUE,expires_at TIMESTAMPTZ NOT NULL,used_at TIMESTAMPTZ,created_at TIMESTAMPTZ NOT NULL DEFAULT NOW());
CREATE TABLE IF NOT EXISTS company_licence_allocations (company_id UUID NOT NULL REFERENCES companies(id) ON DELETE CASCADE,licence_id UUID NOT NULL REFERENCES licences(id) ON DELETE CASCADE,seats INTEGER NOT NULL DEFAULT 0 CHECK(seats>=0),assigned_by UUID REFERENCES users(id) ON DELETE SET NULL,updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),PRIMARY KEY(company_id,licence_id));
CREATE TABLE IF NOT EXISTS user_licence_assignments (user_id UUID PRIMARY KEY REFERENCES users(id) ON DELETE CASCADE,company_id UUID NOT NULL REFERENCES companies(id) ON DELETE CASCADE,licence_id UUID NOT NULL REFERENCES licences(id) ON DELETE RESTRICT,assigned_by UUID REFERENCES users(id) ON DELETE SET NULL,assigned_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),active BOOLEAN NOT NULL DEFAULT TRUE,starts_at TIMESTAMPTZ,expires_at TIMESTAMPTZ,CHECK(expires_at IS NULL OR starts_at IS NULL OR expires_at >= starts_at));
