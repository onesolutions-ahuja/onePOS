...
export default function CustomReportsAdmin() {
  ...
  useEffect(() => {
    ...
    getPlatformReportFields(definition.objectId)
      .then((response) => {
      if (!cancelled) {
        const directFields = response.data?.fields || [];
        const relatedFields = (response.data?.relationships || []).flatMap((relationship) => (
          (relationship.fields || []).map((field) => ({
            ...field,
            key: `${relationship.relationship_key}.${field.key || field.api_name}`,
            label: `${relationship.label || relationship.relationship_key} · ${field.label || field.api_name}`,
            groupable: field.groupable !== false,
          }))
        ));
        setPlatformFields([...directFields, ...relatedFields]);
      }
    })
      .catch(() => {
