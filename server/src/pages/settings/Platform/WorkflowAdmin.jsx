...
function StepEditor({ step, index, updateStep, moveStep, duplicateStep, deleteStep, addStepAt, providerAvailable, registryOptions, rootObjectKey }) {
...
function WorkflowCanvas({ workflow, setWorkflow, updateStep, moveStep, duplicateStep, deleteStep, addStepAt, providerAvailable, registryOptions }) {
...
export default function WorkflowAdmin({ onMessage, onError }) {
  ...
  const [registryOptions, setRegistryOptions] = useState(actionOptions);

  useEffect(() => {
    apiRequest("/api/platform/workflow-actions")
      .then((response) => {
        const registry = (response.data || []).map((item) => ({ value: item.key, label: item.displayName || item.key }));
  ...
  useEffect(() => {
    apiRequest("/api/platform/rules")
      .then((response) => {
        ...
        const workflows = rules.filter((rule) => rule?.action?.type === "workflow").map((rule) => ({
          ...rule,
          id: rule.id,
          name: rule.name,
              ...
              <div className="flex gap-3">
                ...
                {item.id ? <button type="button" className="text-sm text-slate-600" onClick={() => {
                  ...
                  apiRequest(`/api/platform/rules/${item.id}`, { method: "PUT", body: JSON.stringify({
                    ...
                    active: nextActive,
                    action: { type: "workflow", match: item.match || "all", actions: (item.steps || []).filter((step) => step.enabled !== false).map((step) => ({ type: step.type, ...(step.config || {}), fieldValues: step.config?.fieldValues || step.config?.fieldMappings })) },
  ...
  return (
    <div className="space-y-4">
      ...
      <WorkflowCanvas workflow={workflow} setWorkflow={setWorkflow} updateStep={updateStep} moveStep={moveStep} duplicateStep={duplicateStep} deleteStep={deleteStep} addStepAt={addStepAt} providerAvailable={providerAvailable} registryOptions={registryOptions} />
    </div>
  );
}
