...
export default function createSupplierAccountsRouter({ authenticate, authorize, db, pool }) {
  ...
  router.post("/supplier-payments", authenticate, authorize("payment.manage", ...manage), async (req, res) => {
    ...
    try {
      ...
      const status = error.code === "DUPLICATE_PAYMENT" || error.code === "23505" ? 409 : error.message === "Supplier not found" ? 404 : 400;
      res.status(status).json({ success: false, message: error.message, ...(error.paymentId ? { data: { id: error.paymentId, duplicate: true } } : {}) });
    } finally { client.release(); }
  });
