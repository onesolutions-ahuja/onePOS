...
export default function createReturnsRouter({
  ...
  async function resolveSaleRef(client, req, rawReference) {
    ...
    const result = await client.query(
      `SELECT id, company_id, store_id, customer_id
       FROM sales
       WHERE company_id = $1
  ...
  async function recordCustomerCreditRefund(client, {
    ...
    await client.query(
      ...
      [
        ...
        `sale_return_credit_refund:${returnId}`,
      ...
      try {
        ...
        if (refundAmount > 0) {
          ...
          for (const part of allocation) {
            ...
            if (part.method === "customer_credit") {
              await recordCustomerCreditRefund(client, {
                companyId: req.user.companyId,
                customerId: loaded.sale.customer_id,
        ...
        const canonicalTransactionId = typeof canonicalTransactionWriter === "function"
          ...
          customerId: loaded.sale.customer_id,
          subtotal: -refundAmount + Number(loaded.sale.tax || 0) * 0,
          tax: 0,
          total: -refundAmount,
          relatedReferenceIds: [returnId],
          lines: items.map((item) => {
            const source = loaded.items.find((row) => row.id === item.saleItemId);
            const quantity = -Number(item.quantity);
            const total = round2((source?.unitRefundValue || 0) * quantity);
            return {
              productId: item.productId,
              productName: source?.productName || "Returned product",
              quantity,
              unitPrice: source?.unitRefundValue || 0,
              tax: 0,
              total,
            };
          }),
        ...
        if (canonicalTransactionId) {
          await client.query(
            ...
            [canonicalTransactionId, returnId]
          );
        }
