  ...
  return (
    <div className="space-y-5">
      <div className="flex items-start justify-between gap-4">
        <div><h1 className="text-2xl font-bold">Layouts</h1><p className="text-sm text-slate-500 mt-1">Configure metadata-driven page layouts.</p></div>
        <button type="button" onClick={onNew} className="h-10 px-4 bg-blue-600 text-white rounded-lg text-sm">+ New Layout</button>
      </div>
      <div className="bg-white border border-slate-200 rounded-xl overflow-x-auto">
        {loading ? <div className="p-8 text-sm text-slate-500">Loading layouts...</div> : layouts.length === 0 ? <div className="p-8 text-sm text-slate-500">No layouts configured.</div> : (
          <table className="w-full text-sm"><thead><tr className="text-left border-b"><th className="p-4">Name</th><th>Object</th><th>Page type</th><th>Role</th><th>Company</th><th>Status</th><th /></tr></thead>
      ...
      {onBack && <button type="button" onClick={onBack} className="text-sm text-slate-600">← Platform</button>}
    </div>
  );
}
