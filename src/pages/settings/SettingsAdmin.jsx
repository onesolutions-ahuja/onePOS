...
function SettingsAdmin({ initialTab = "General", user = null, isAdmin = false, isSuperadmin = false, entitlements = {} }) {
  ...
  const access = settingSectionAccess({
    isAdmin,
    isSuperadmin,
    isPlatformDeveloper: user?.isPlatformDeveloper === true,
    ...
    <div className="settings-workspace">
      ...
      <div className="settings-content">
        ...
        {tab === "Users" && <>
          ...
          <AccountPolicySettings onMessage={setMessage} onError={setError} />
        </>}
...
function UsersSettings({ onMessage, onError }) {
  ...
  const toggle = async (user) => {
    try {
      const data = await apiRequest(`/api/platform/objects/employee/records/${encodeURIComponent(user.id)}`, {
        method: "PUT",
        body: JSON.stringify({ data: { active: !user.active } }),
      });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="onepos-card overflow-hidden">
        ...
        {form && <UserFormModal form={form} onClose={() => setForm(null)} onSaved={recordSaved} />}
      </div>
    </div>
  );
}
...
function RoleObjectPermissions({ roleId, onMessage, onError }) {
  ...
  useEffect(() => {
    if (!roleId) return;
    setLoading(true);
    apiRequest(`/api/admin/roles/${roleId}/object-permissions`)
      .then((response) => setRows(response.data || []))
      .catch((error) => onError(error.message || "Unable to load object permissions"))
      .finally(() => setLoading(false));
  }, [roleId, onError]);
  const update = (index, key) => setRows((current) => current.map((row, rowIndex) => rowIndex === index ? { ...row, [key]: !row[key] } : row));
  const save = async () => {
    setSaving(true);
    try {
      ...
      onError(error.message || "Unable to save object permissions");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="onepos-card overflow-hidden">
      <div className="flex items-center justify-between gap-3 border-b bg-slate-50 px-4 py-3">
        <div><h4 className="font-medium text-slate-700">Platform Object Permissions</h4><p className="text-xs text-slate-500">These server-enforced permissions control platform record CRUD.</p></div>
              ...
              {PERMISSION_GROUPS.map((group) => (
                <div key={group.label} className="onepos-card overflow-hidden">
                  ...
                  <div className="p-3 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2">
                    ...
                    <RoleObjectPermissions roleId={selectedRoleId} onMessage={onMessage} onError={onError} />
                  </div>
                </div>
              ))}
            </div>
...
function StoreEditRow({ store, onSave }) {
  const dialog = useSettingsEditDialog();
  const [form, setForm] = useState({ name: store.name, code: store.code || "", addressLine1: store.address_line1 || "", city: store.city || "", postcode: store.postcode || "", phone: store.phone || "", active: store.active });
  return <><SettingsDetails title={store.name || "Store"} onEdit={dialog.openEdit} fields={[{ label: "Code", value: store.code }, { label: "City", value: store.city }, { label: "Phone", value: store.phone }, { label: "Status", value: store.active ? "Active" : "Inactive" }]} />
    <SettingsEditDialog open={dialog.open} title={`Edit ${store.name || "Store"}`} onClose={dialog.closeEdit}><div className="grid grid-cols-2 gap-3">
      <label className="text-sm">Name<input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="onepos-input mt-1" /></label>
      <label className="text-sm">Code<input value={form.code} onChange={(e) => setForm({ ...form, code: e.target.value })} className="onepos-input mt-1" /></label>
      <label className="text-sm">City<input value={form.city} onChange={(e) => setForm({ ...form, city: e.target.value })} className="onepos-input mt-1" /></label>
      <label className="text-sm">Phone<input value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} className="onepos-input mt-1" /></label>
      <label className="col-span-2 flex items-center gap-2 text-sm"><Toggle checked={form.active} onChange={(e) => setForm({ ...form, active: e.target.checked })} /> Active</label>
    </div><div className="flex justify-end gap-2 mt-4"><button type="button" className="onepos-btn onepos-btn-secondary" onClick={dialog.closeEdit}>Cancel</button><button type="button" className="onepos-btn onepos-btn-primary" onClick={async () => { await onSave({ id: store.id, ...form }); dialog.closeEdit(); }}>Save store</button></div></SettingsEditDialog></>;
}
function TillEditRow({ till, onSave }) {
  const dialog = useSettingsEditDialog();
  const [form, setForm] = useState({ name: till.name, terminalNumber: till.terminalNumber || "", active: till.active });
  return <><div className="settings-related-detail"><SettingsDetails title={till.name || "Till"} onEdit={dialog.openEdit} fields={[{ label: "Terminal number", value: till.terminalNumber }, { label: "Status", value: till.active ? "Active" : "Inactive" }]} /></div>
    <SettingsEditDialog open={dialog.open} title={`Edit ${till.name || "Till"}`} onClose={dialog.closeEdit}><div className="grid gap-3"><label className="text-sm">Name<input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="onepos-input mt-1" /></label><label className="text-sm">Terminal number<input value={form.terminalNumber} onChange={(e) => setForm({ ...form, terminalNumber: e.target.value })} className="onepos-input mt-1" /></label><label className="flex items-center gap-2 text-sm"><Toggle checked={form.active} onChange={(e) => setForm({ ...form, active: e.target.checked })} /> Active</label></div><div className="flex justify-end gap-2 mt-4"><button type="button" className="onepos-btn onepos-btn-secondary" onClick={dialog.closeEdit}>Cancel</button><button type="button" className="onepos-btn onepos-btn-primary" onClick={async () => { await onSave({ id: till.id, ...form }); dialog.closeEdit(); }}>Save till</button></div></SettingsEditDialog></>;
}
...
const companyFields = <>{field("companyName", "Company name")}{field("legalName", "Legal / business name")}{field("companyEmail", "Email", "email")}{field("companyPhone", "Phone")}{field("currency", "Currency")}{field("timezone", "Timezone")}<LogoUploader logoUrl={form.logoUrl || ""} onChange={(value) => setForm((current) => ({ ...current, logoUrl: value }))} /></>;
  ...
  const values = tab === "Company"
    ? [["Company name", form.companyName], ["Legal / business name", form.legalName], ["Email", form.companyEmail], ["Phone", form.companyPhone], ["Currency", form.currency], ["Timezone", form.timezone]]
    : tab === "Tax / VAT"
      ? [["VAT", form.vatEnabled ? "Enabled" : "Disabled"], ["Default VAT rate", `${form.defaultVatRate || 0}%`], ["Currency", form.currency]]
      : [["Date format", form.dateFormat], ["Currency", form.currency], ["Timezone", form.timezone], ["Scan & Go", form.scanGoEnabled ? "Enabled" : "Disabled"], ["Exchange mode", ({ receipt: "Receipt / Invoice only", normal: "Normal / No receipt only", both: "Both — cashier chooses" })[form.exchangeMode] || "Both — cashier chooses"]];
  return <>
    <SettingsDetails title={tab} onEdit={dialog.openEdit} fields={values.map(([label, value]) => ({ label, value }))} />
    <SettingsEditDialog open={dialog.open} title={`Edit ${tab}`} onClose={dialog.closeEdit}>
      <form onSubmit={async (event) => { await onSave(event); dialog.closeEdit(); }} className="space-y-4">
        <div className="grid grid-cols-2 gap-4">{tab === "Company" ? companyFields : <>{field("dateFormat", "Date format")}{field("currency", "Currency")}{field("timezone", "Timezone")}{tab === "Tax / VAT" && <><label className="flex items-center gap-2 text-sm text-slate-600 pt-6"><Toggle checked={form.vatEnabled} onChange={(event) => setForm((current) => ({ ...current, vatEnabled: event.target.checked }))} /> VAT enabled</label>{field("defaultVatRate", "Default VAT rate %", "number")}</>}{tab === "General" && <><label className="flex items-center gap-2 text-sm text-slate-600 pt-6"><Toggle checked={form.scanGoEnabled === true} onChange={(event) => setForm((current) => ({ ...current, scanGoEnabled: event.target.checked }))} /> Scan &amp; Go</label><label className="text-sm text-slate-600"><span className="block mb-1 font-medium">Exchange mode</span><select value={form.exchangeMode || "both"} onChange={(event) => setForm((current) => ({ ...current, exchangeMode: event.target.value }))} className="onepos-input"><option value="receipt">Receipt / Invoice only</option><option value="normal">Normal / No receipt only</option><option value="both">Both — cashier chooses</option></select></label></>}</>}</div>
        <div className="flex justify-end gap-2"><button type="button" className="onepos-btn onepos-btn-secondary" onClick={dialog.closeEdit}>Cancel</button><button type="submit" className="onepos-btn onepos-btn-primary">Save settings</button></div>
      </form>
    </SettingsEditDialog>
  </>;
}
...
function ReceiptSettings({ settings, form, setForm, onSave }) {
  return <SettingsDetails title="Receipt settings" description="Receipts use the existing company and tax settings. Printer configuration is managed under Hardware." fields={[{ label: "Header company", value: settings.company.name }, { label: "VAT display", value: form.vatEnabled ? `Enabled (${form.defaultVatRate}%)` : "Disabled" }, { label: "Date format", value: form.dateFormat }, { label: "Paper width", value: "Configure under Hardware" }]} />;
}
...
function LoyaltySettings({ settings, form, setForm, onSave }) {
  ...
  const editor = (
    <form onSubmit={async (event) => { const saved = await handleSave(event); if (saved) dialog.closeEdit(); }} className="space-y-5">
      ...
      <div className="flex justify-end gap-2 mt-6">
        <button type="button" className="onepos-btn onepos-btn-secondary" onClick={dialog.closeEdit}>Cancel</button>
  ...
  return <>
    <SettingsDetails title="Customer Loyalty" description="Configure the customer loyalty programme." onEdit={dialog.openEdit}
      fields={[{ label: "Programme", value: form.loyaltyEnabled !== false ? "Enabled" : "Disabled" }, { label: "Earning rate", value: `${formatRate(form.loyaltyEarningRate)}%` }]} />
    <SettingsEditDialog open={dialog.open} title="Edit Customer Loyalty" onClose={dialog.closeEdit}>{editor}</SettingsEditDialog>
  </>;
}
...
function PaymentTerminalSettings({ terminals, onSaved, onMessage, onError }) {
  ...
  const editor = <form onSubmit={async (event) => { await save(event); dialog.closeEdit(); }} className="space-y-4"><div className="grid grid-cols-2 gap-4">{[["provider", "Provider"], ["name", "Terminal name"], ["terminalIdentifier", "Terminal ID"], ["connectionUrl", "Connection/API URL"]].map(([name, label]) => <label key={name} className="text-sm text-slate-600"><span className="block mb-1 font-medium">{label}</span><input required={name !== "connectionUrl"} value={form[name]} onChange={(event) => setForm((current) => ({ ...current, [name]: event.target.value }))} className="onepos-input" /></label>)}<label className="text-sm text-slate-600 col-span-2"><span className="block mb-1 font-medium">API credentials</span><input type="password" value={form.apiCredentials} onChange={(event) => setForm((current) => ({ ...current, apiCredentials: event.target.value }))} placeholder={form.has_credentials ? "Leave blank to keep existing credentials" : "Stored securely on the server"} className="onepos-input" /></label></div><div className="flex justify-end gap-2"><button type="button" className="onepos-btn onepos-btn-secondary" onClick={dialog.closeEdit}>Cancel</button><button className="onepos-btn onepos-btn-primary">Save terminal</button></div></form>;
  return <div className="space-y-5"><SettingsDetails title="Payment terminals" description={`${terminals.length} terminal${terminals.length === 1 ? "" : "s"} configured.`} onEdit={() => { setForm({ provider: "", name: "", terminalIdentifier: "", connectionUrl: "", apiCredentials: "" }); dialog.openEdit(); }} fields={[{ label: "Configured", value: terminals.length }, { label: "Active", value: terminals.filter((terminal) => terminal.active).length }]} /><SettingsEditDialog open={dialog.open} title={form.id ? "Edit Payment Terminal" : "Add Payment Terminal"} onClose={dialog.closeEdit}>{editor}</SettingsEditDialog><div className="onepos-card overflow-hidden"><table className="w-full"><thead><tr className="bg-slate-50">{["Provider", "Name", "Terminal ID", "Credentials", "Status", "Actions"].map((heading) => <th key={heading} className="text-left px-4 py-3 text-xs font-semibold text-slate-500 uppercase">{heading}</th>)}</tr></thead><tbody>{terminals.map((terminal) => <tr key={terminal.id} className="border-t border-slate-100"><td className="px-4 py-3 text-sm">{terminal.provider}</td><td className="px-4 py-3 text-sm">{terminal.name}</td><td className="px-4 py-3 text-sm">{terminal.terminal_identifier || "-"}</td><td className="px-4 py-3 text-sm">{terminal.has_credentials ? "Configured (masked)" : "Not configured"}</td><td className="px-4 py-3 text-sm">{terminal.active ? "Active" : "Inactive"}</td><td className="px-4 py-3 whitespace-nowrap"><button onClick={() => { setForm({ id: terminal.id, provider: terminal.provider, name: terminal.name, terminalIdentifier: terminal.terminal_identifier || "", connectionUrl: terminal.connection_url || "", apiCredentials: "", has_credentials: terminal.has_credentials }); dialog.openEdit(); }} className="px-3 py-2 text-slate-600 bg-slate-100 rounded-lg text-sm mr-1">Edit</button><button onClick={() => test(terminal.id)} className="px-3 py-2 bg-slate-100 rounded-lg text-sm">Test Connection</button></td></tr>)}</tbody></table></div></div>;
}
...
function HardwareCard({ type, label, current, onSave, onTest }) {
  ...
  const editor = <div><div className="grid grid-cols-2 gap-4"><label className="text-sm text-slate-600"><span className="block mb-1 font-medium">Device name</span><input value={form.deviceName} onChange={(event) => setForm((currentForm) => ({ ...currentForm, deviceName: event.target.value }))} className="onepos-input" /></label><label className="text-sm text-slate-600"><span className="block mb-1 font-medium">Connection type</span><select value={form.connectionType} onChange={(event) => setForm((currentForm) => ({ ...currentForm, connectionType: event.target.value }))} className="onepos-input"><option value="">Not configured</option><option>Keyboard / HID</option><option>Local service</option><option>Network</option><option>USB / Serial</option></select></label></div>{type === "RECEIPT_PRINTER" && <label className="block mt-4 text-sm text-slate-600"><span className="block mb-1 font-medium">Paper width</span><select value={form.paperWidth} onChange={(event) => setForm((currentForm) => ({ ...currentForm, paperWidth: event.target.value }))} className="onepos-input w-40"><option value="">Not set</option><option>58mm</option><option>80mm</option></select></label>}{scanner && <div className="mt-4 p-3 bg-slate-50 rounded-lg text-sm"><div className="font-medium">Scanner test area</div><div className="text-slate-500 mt-1">Last scanned barcode: <span className="font-semibold text-slate-800">{scan || "-"}</span></div><div className="text-slate-500">Scan time: {scanTime || "-"}</div><div className="text-slate-500">Scanner status: {scan ? "Scan received" : "Waiting for keyboard/HID scan"}</div></div>}<label className="flex items-center gap-2 mt-4 text-sm text-slate-600"><Toggle checked={form.active} onChange={(event) => setForm((currentForm) => ({ ...currentForm, active: event.target.checked }))} /> Configured and active</label><div className="flex justify-end gap-2 mt-4"><button type="button" className="onepos-btn onepos-btn-secondary" onClick={dialog.closeEdit}>Cancel</button><button onClick={async () => { await onSave(form); dialog.closeEdit(); }} className="onepos-btn onepos-btn-primary">Save configuration</button></div></div>;
  return <><SettingsDetails title={label} description={current.last_test_result || (current.active ? "Configured" : "Not configured")} onEdit={dialog.openEdit} fields={[{ label: "Device", value: current.device_name }, { label: "Connection", value: current.connection_type }, ...(type === "RECEIPT_PRINTER" ? [{ label: "Paper width", value: current.paper_width }] : []), { label: "Status", value: current.active ? "Active" : "Inactive" }]}><div className="mt-4"><button onClick={() => onTest(type)} className="onepos-btn onepos-btn-secondary">{type === "CASH_DRAWER" ? "Test Open Drawer" : type === "RECEIPT_PRINTER" ? "Test Print" : "Test"}</button></div></SettingsDetails><SettingsEditDialog open={dialog.open} title={`Edit ${label}`} onClose={dialog.closeEdit}>{editor}</SettingsEditDialog></>;
}
    ...
    return <div key={platform.platform} className="onepos-card onepos-card-body">
      <SettingsDetails
        ...
        fields={[
          ...
          { label: "Environment", value: platform.environment || "Sandbox" },
          { label: "Client ID", value: platform.client_id },
          { label: "Store / location ID", value: platform.store_location_id },
          { label: "Store ID", value: platform.store_id },
          { label: "Brand ID", value: platform.brand_id },
          { label: "Order acceptance", value: platform.order_acceptance === "auto" ? "Automatic" : "Manual" },
          { label: "Require OTP on completion", value: platform.require_otp_on_completion ? "Yes" : "No" },
          { label: "Notes", value: platform.notes },
        ]}
      />
