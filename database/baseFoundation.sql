...
UPDATE package_registry
   SET licence_mode=CASE
     WHEN package_type='FOUNDATION' OR billable=false THEN 'TECHNICAL'
     ELSE 'COMMERCIAL'
   END
...
CREATE TABLE IF NOT EXISTS company_package_entitlement_sources (
  ...
  company_id UUID NOT NULL REFERENCES companies(id) ON DELETE CASCADE,
  package_id UUID NOT NULL REFERENCES package_registry(id) ON DELETE CASCADE,
  source_type VARCHAR(30) NOT NULL CHECK(source_type IN ('DIRECT_LICENCE','DIRECT_INSTALL','BUNDLE','TIER','REQUIRED_DEPENDENCY','OPTIONAL_DEPENDENCY','PLATFORM_DEFAULT','SUPERADMIN_ASSIGNMENT')),
