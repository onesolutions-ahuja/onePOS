 ...
 * Creates fully-valid UUID parent rows (company -> store -> roles/users),
...
const createTestDb = async () => {
  ...
  const cat = await db(`INSERT INTO categories(company_id,name) VALUES($1,$2) RETURNING id`, [companyId, `LowStockTest-${tag}`]);
