...
test("install and reinstall reuse the Supplier object and preserve tenant custom metadata and sourcing links", async () => {
  ...
  const db = async (sql, params = []) => {
    ...
    if (sql.startsWith("SELECT id,company_id FROM platform_fields")) {
      ...
      return { rows: field ? [{ id: field.id, company_id: field.company_id }] : [] };
    }
    if (sql.startsWith("INSERT INTO platform_fields")) {
      const [, apiName, , , , , , , , companyId] = params;
      const id = `field-${params[0]}-${apiName}-${companyId || "global"}`;
      fields.set(`${params[0]}:${apiName}:${companyId || "global"}`, { id, company_id: companyId || null, source_column: params[13] });
      return { rows: [] };
    }
  ...
  assert.equal(requiredSupplierField.params[10], true);
  assert.match(requiredSupplierField.sql, /package_required=\$11/);
  assert.ok(calls.filter(({ sql }) => sql.startsWith("INSERT INTO platform_fields")).every(({ params, sql }) =>
    sql.includes("source_column") && params[13] === params[1]
  ));
  assert.ok(calls.filter(({ sql }) => sql.startsWith("SELECT id,company_id FROM platform_fields")).some(({ params }) => params[2] === "company-b"));
...
test("installing Purchasing succeeds through the Supplier Core and Product Core dependency plan", async (t) => {
  ...
  for (const object of supplierCore.manifest.objects) {
    ...
    for (const field of object.fields) fieldKeys.add(`${objectId}:${field.apiName}:global`);
  }
  const calls = [];
  const db = async (sql, params = []) => {
    calls.push({ sql, params });
    ...
    if (sql.startsWith("SELECT id,version,package_type,installable FROM package_registry")) {
      const item = packages.find((entry) => entry.package_key === params[0]);
      return { rows: item ? [{ id: item.id, version: item.version, package_type: item.package_type, installable: item.installable }] : [] };
    }
    if (sql.startsWith("SELECT id,package_id,module_id,company_id FROM platform_objects")) {
      const object = objects.get(params[0]);
      return { rows: object ? [object] : [] };
    ...
    if (sql.startsWith("SELECT id,company_id FROM platform_fields")) {
      ...
      return { rows: fieldKeys.has(key) ? [{ id: key, company_id: null }] : [] };
    }
    ...
    if (sql.startsWith("SELECT id FROM platform_fields WHERE object_id=$1 AND api_name=$2")) {
      return { rows: fieldKeys.has(`${params[0]}:${params[1]}:global`) ? [{ id: params[1] }] : [] };
    }
  ...
  assert.equal(calls.filter(({ sql }) => sql.startsWith("INSERT INTO platform_relationships")).length, 4);
});
