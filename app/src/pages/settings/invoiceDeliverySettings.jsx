...
import { SettingsDetails, SettingsEditDialog, useSettingsEditDialog } from "../../components/settings/SettingsDetails.jsx";
...
function InvoiceDeliverySettings({ channel, onMessage, onError }) {
  ...
  return (
    <div className="space-y-5 max-w-3xl">
      <SettingsDetails
        title={`${config.label} invoice delivery`}
        description="Saved configuration and delivery status."
        onEdit={dialog.openEdit}
        fields={[
          { label: "Status", value: enabled ? "Enabled" : "Disabled" },
          { label: "Configuration", value: state.configured ? "Configured" : "Not configured" },
          ...config.fields.map(([formKey, cfgKey, label]) => ({ label, value: state[cfgKey] })),
          { label: "Credentials", value: tokenConfigured ? (state.auth_token_masked || state.api_key_masked || "Stored (masked)") : "Not configured" },
          { label: "Automatic sending", value: state.auto_send_enabled ? "On" : "Off" },
        ]}
      />
      <SettingsEditDialog open={dialog.open} title={`Edit ${config.label} invoice delivery`} onClose={dialog.closeEdit}>
