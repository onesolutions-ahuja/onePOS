...
export function validateCustomReportDefinition(input = {}) {
  ...
  return {
    ...
    storeIds: validateUuidList(definition.storeIds, "store"),
    userIds: validateUuidList(definition.userIds, "user"),
  };
}
...
export default function createReportsRouter({ authenticate, authorize, db }) {
  ...
  async function platformReportContext(req, objectId) {
    ...
    if (!object) return { object: null, fields: [], relationships: [] };
    ...
    let fields = fieldResult.rows;
    if (req.user.roleId && fields.length) {
      const security = await db("SELECT field_id, readable FROM platform_field_security WHERE field_id=ANY($1::uuid[]) AND role_id=$2 AND company_id=$3", [fields.map((field) => field.id), req.user.roleId, req.user.companyId]);
      const denied = new Set(security.rows.filter((row) => row.readable === false).map((row) => String(row.field_id)));
      fields = fields.filter((field) => field.readable !== false && !denied.has(String(field.id)));
    } else {
      fields = fields.filter((field) => field.readable !== false);
    }
    const relationshipResult = await db(`SELECT rel.relationship_key, rel.relationship_type,
        target.source_table AS target_source_table, child.source_column AS child_source_column,
    ...
    for (const relationship of relationshipResult.rows) {
      const relatedFields = await db("SELECT * FROM platform_fields WHERE object_id=$1 AND active=true AND readable=true AND (company_id IS NULL OR company_id=$2) ORDER BY display_order, label", [relationship.target_object_id, req.user.companyId]);
      relationships.push({
        ...relationship,
        local_source_column: relationship.child_source_column,
        target_source_column: "id",
        fields: relatedFields.rows,
      });
    }
    return { object, fields, relationships };
  }
  ...
  router.post("/reports/custom", authenticate, authorize("reports.custom.create"), async (req, res) => {
    try {
      ...
      if (definition.dataSource === "platform_object") {
        ...
        validatePlatformReportDefinition(req.body, context.object, context.fields, context.relationships);
      }
  ...
  router.put("/reports/custom/:id/users", authenticate, authorize("reports.custom.share"), async (req, res) => {
    try {
      ...
      const userIds = validateUuidList(req.body?.userIds, "user");
  ...
  router.post("/reports/custom/preview", authenticate, authorize("reports.custom.view"), async (req, res) => {
    try {
      ...
      if (definition.dataSource === "platform_object") {
        const context = await platformReportContext(req, definition.objectId);
        const built = buildPlatformObjectQuery(definition, context.object, context.fields, req.user.companyId, 100, { storeId: req.user.storeId }, context.relationships);
        const result = await db(built.sql, built.params);
        return res.json({ success: true, data: { columns: definition.fields.map((key) => ({ key, label: context.fields.find((field) => field.api_name === key)?.label || key })), rows: result.rows } });
      }
  ...
  router.post("/reports/custom/:id/run", authenticate, authorize("reports.custom.view"), async (req, res) => {
    try {
      ...
      // Platform-object reports enforce their own object/store scope in
      // buildPlatformObjectQuery. Do not force the legacy sales-store scope
      // before we know which datasource is being executed.
      if (definition.dataSource === "platform_object") {
        const context = await platformReportContext(req, definition.objectId);
        const built = buildPlatformObjectQuery(definition, context.object, context.fields, req.user.companyId, 1000, { storeId: req.user.storeId }, context.relationships);
        const result = await db(built.sql, built.params);
        const columns = definition.fields.map((key) => ({ key, label: context.fields.find((field) => field.api_name === key)?.label || key }));
