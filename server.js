...
app.use("/api", createDashboardBuilderRouter({ authenticate, authorize, db, canViewCompanyCustomers }));
    ...
    if (pool) {
      ...
      try {
        ...
        if (process.env.PLATFORM_DEVELOPER_PASSWORD) {
          ...
          await db(
            `INSERT INTO users (company_id,username,password_hash,full_name,email,is_platform_developer,active)
             VALUES (NULL,'developer@onepos.local',$1,'Platform Developer','developer@onepos.local',true,true)
             ON CONFLICT (username) DO UPDATE SET is_platform_developer=true,active=true,password_hash=EXCLUDED.password_hash`,
            [developerHash]
          );
          await db(
            `UPDATE users
             SET role_id=(SELECT id FROM roles WHERE company_id IS NULL AND name='Platform Developer' LIMIT 1)
             WHERE username='developer@onepos.local' AND is_platform_developer=true`
          );
        }
